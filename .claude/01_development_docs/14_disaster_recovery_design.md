...
- [1. 概要](#1-概要)
  ...
  - [1.4 基本方針（更新版）](#14-基本方針更新版)
- [2. 通常運用フロー（更新版）](#2-通常運用フロー更新版)
  - [2.1 スマホ注文フロー](#21-スマホ注文フロー)
  - [2.2 ハンディ注文フロー](#22-ハンディ注文フロー)
  - [2.3 障害時の動作](#23-障害時の動作)
- [3. 障害復旧の実装設計](#3-障害復旧の実装設計)
  - [3.1 システムモード管理](#31-システムモード管理)
  - [3.2 FireBirdテーブル設計](#32-firebirdテーブル設計)
  - [3.3 障害検知と復旧処理](#33-障害検知と復旧処理)
  - [3.4 注文処理の実装](#34-注文処理の実装)
- [4. 障害復旧同期フロー（最終版）](#4-障害復旧同期フロー最終版)
  - [4.1 同期フェーズ](#41-同期フェーズ)
  - [4.2 シンプル同期API](#42-シンプル同期api)
  - [4.3 実装の特徴](#43-実装の特徴)
- [5. 運用シナリオ](#5-運用シナリオ)
  - [5.1 障害発生から復旧まで](#51-障害発生から復旧まで)
  - [5.2 メリットと効果](#52-メリットと効果)
- [5. 具体的な障害復旧シナリオ](#5-具体的な障害復旧シナリオ)
  - [5.1 障害発生から復旧までの流れ](#51-障害発生から復旧までの流れ)
  - [5.2 実装後の効果](#52-実装後の効果)
- [6. 実装規模・工数](#6-実装規模工数)
  - [6.1 変更規模](#61-変更規模)
  - [6.2 テスト項目](#62-テスト項目)
- [7. 運用手順](#7-運用手順)
  ...
  - [7.2 復旧時](#72-復旧時)
  - [7.3 トラブル時](#73-トラブル時)
- [3.3 同期API拡張](#33-同期api拡張)
- [4. 復旧時の差分同期手順](#4-復旧時の差分同期手順)
  - [4.1 同期フェーズ](#41-同期フェーズ)
  - [4.2 競合解決ルール](#42-競合解決ルール)
  - [4.3 同期完了処理](#43-同期完了処理)
- [5. 技術実装詳細](#5-技術実装詳細)
  - [5.1 FireBird対応のデータ操作](#51-firebird対応のデータ操作)
  - [5.2 データベーストランザクション管理](#52-データベーストランザクション管理)
  - [5.2 べき等性保証](#52-べき等性保証)
  - [5.3 エラーハンドリング](#53-エラーハンドリング)
- [6. 運用手順](#6-運用手順)
  - [6.1 障害発生時対応](#61-障害発生時対応)
  - [6.2 復旧時対応](#62-復旧時対応)
  - [6.3 監視ポイント](#63-監視ポイント)
- [7. テスト設計](#7-テスト設計)
  - [7.1 障害シミュレーション](#71-障害シミュレーション)
...
```
ヘルスチェック判定:
- 30秒無応答 → warning状態（通信混雑メッセージ）
- 90秒無応答 → error状態（システムエラーメッセージ）

処理:
1. 30秒障害 → 新規注文一時停止（お客様画面に「通信混雑」表示）
2. 90秒障害 → 新規注文停止（お客様画面に「システムエラー」表示）
